...
DEVICE MS__Device_Table =
  {
    MS__EP_NUM,
    1
  };

DEVICE_PROP MS__Device_Property =
  {
    MASS_init,
    MASS_Reset,
    MASS_Status_In,
...
USER_STANDARD_REQUESTS MS__User_Standard_Requests =
  {
...
ONE_DESCRIPTOR MS__Device_Descriptor =
  {
    (uint8_t*)MASS_DeviceDescriptor,
    MASS_SIZ_DEVICE_DESC
  };

ONE_DESCRIPTOR MS__Config_Descriptor =
  {
    (uint8_t*)MASS_ConfigDescriptor,
    MASS_SIZ_CONFIG_DESC
  };

ONE_DESCRIPTOR MS__String_Descriptor[5] =
  {
...
void MASS_Reset()
{
  ...
  SetBTABLE(MS__BTABLE_ADDRESS);

  /* Initialize Endpoint 0 */
  SetEPType(ENDP0, EP_CONTROL);
  SetEPTxStatus(ENDP0, EP_TX_NAK);
  SetEPRxAddr(ENDP0, MS__ENDP0_RXADDR);
  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);
  SetEPTxAddr(ENDP0, MS__ENDP0_TXADDR);
  Clear_Status_Out(ENDP0);
  SetEPRxValid(ENDP0);

  /* Initialize Endpoint 1 */
  SetEPType(ENDP1, EP_BULK);
  SetEPTxAddr(ENDP1, MS__ENDP1_TXADDR);
  SetEPTxStatus(ENDP1, EP_TX_NAK);
  SetEPRxStatus(ENDP1, EP_RX_DIS);

  /* Initialize Endpoint 2 */
  SetEPType(ENDP2, EP_BULK);
  SetEPRxAddr(ENDP2, MS__ENDP2_RXADDR);
 ...
 * Description    : Initialise USB structure with MS values.
 * Input          : None.
 * Output         : None.
 * Return         : None.
 *******************************************************************************/
...
{
	IMR_MSK = MS__IMR_MSK;
	Device_Table = MS__Device_Table;
	Device_Property = MS__Device_Property;
	User_Standard_Requests = MS__User_Standard_Requests;
	Device_Descriptor = MS__Device_Descriptor;
	Config_Descriptor = MS__Config_Descriptor;
	String_Descriptor = MS__String_Descriptor;
	return;
}
