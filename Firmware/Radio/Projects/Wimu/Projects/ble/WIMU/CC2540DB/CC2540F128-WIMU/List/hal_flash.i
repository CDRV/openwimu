#line 1 "D:\\Simon_Dev\\WIMuGPS\\Workspace\\openwimu\\Firmware\\Radio\\Projects\\Wimu\\Components\\hal\\target\\CC2540EB\\hal_flash.c"
/**************************************************************************************************
...
/*******************************************************************************
*/
#line 46 "D:\\Simon_Dev\\WIMuGPS\\Workspace\\openwimu\\Firmware\\Radio\\Projects\\Wimu\\Components\\hal\\target\\CC2540EB\\hal_flash.c"
...
/******************************************************************************
******************************************************************************/
#line 47 "D:\\Simon_Dev\\WIMuGPS\\Workspace\\openwimu\\Firmware\\Radio\\Projects\\Wimu\\Components\\hal\\target\\CC2540EB\\hal_flash.c"
...
/**************************************************************************************************
*/
#line 48 "D:\\Simon_Dev\\WIMuGPS\\Workspace\\openwimu\\Firmware\\Radio\\Projects\\Wimu\\Components\\hal\\target\\CC2540EB\\hal_flash.c"
...
void HalFlashRead(uint8 pg, uint16 offset, uint8 *buf, uint16 cnt)
{
  ...
  do { is = EA; do { EA = 0; } while (83 == -1); } while (83 == -1);
...
void HalFlashWrite(uint16 addr, uint8 *buf, uint16 cnt)
{
  ...
  do { ch->srcAddrH = (uint8)((uint16)(buf) >> 8); ch->srcAddrL = (uint8)((uint16)(buf) & 0xFF); } while (124 == -1);
  do { ch->dstAddrH = (uint8)((uint16)(&((unsigned char volatile __xdata *) 0)[0x6273]) >> 8); ch->dstAddrL = (uint8)((uint16)(&((unsigned char volatile __xdata *) 0)[0x6273]) & 0xFF); } while (125 == -1);
  do { ch->xferLenV &= ~0xE0; ch->xferLenV |= (0x00 << 5); } while (126 == -1);
  do { ch->xferLenL = (uint8)(uint16)((cnt * 4)); ch->xferLenV &= ~0x1F; ch->xferLenV |= (uint8)((uint16)((cnt * 4)) >> 8); } while (127 == -1);
  do { ch->ctrlA &= ~0x80; ch->ctrlA |= (0x00 << 7); } while (128 == -1);
  do { ch->ctrlA &= ~0x60; ch->ctrlA |= (0x00 << 5); } while (129 == -1);
  do { ch->ctrlA &= ~0x1F; ch->ctrlA |= 18; } while (130 == -1);
  do { ch->ctrlB &= ~0xC0; ch->ctrlB |= (0x01 << 6); } while (131 == -1);
  do { ch->ctrlB &= ~0x30; ch->ctrlB |= (0x00 << 4); } while (132 == -1);
  // The DMA is to be polled and shall not issue an IRQ upon completion.
  do { ch->ctrlB &= ~0x08; ch->ctrlB |= (0x00 << 3); } while (134 == -1);
  do { ch->ctrlB &= ~0x04; ch->ctrlB |= (0x00 << 2); } while (135 == -1);
  do { ch->ctrlB &= ~0x03; ch->ctrlB |= 0x02; } while (136 == -1);
  DMAIRQ &= ~( 1 << (0) );
  DMAARM = (0x01 << (0));

  ((unsigned char volatile __xdata *) 0)[0x6271] = (uint8)addr;
  ((unsigned char volatile __xdata *) 0)[0x6272] = (uint8)(addr >> 8);
  ((unsigned char volatile __xdata *) 0)[0x6270] |= 0x02;         // Trigger the DMA writes.
  while (((unsigned char volatile __xdata *) 0)[0x6270] & 0x80);  // Wait until writing is done.
...
void HalFlashErase(uint8 pg)
{
  ((unsigned char volatile __xdata *) 0)[0x6272] = pg * (2048 / 4 / 256);
  ((unsigned char volatile __xdata *) 0)[0x6270] |= 0x01;
}
