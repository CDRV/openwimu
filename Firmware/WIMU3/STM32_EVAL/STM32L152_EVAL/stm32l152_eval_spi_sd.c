...
SD_Error SD_ReadBlock(uint8_t* pBuffer, uint64_t ReadAddr, uint16_t BlockSize)
{
  ...
  SD_SendCmd(SD_CMD_READ_SINGLE_BLOCK, (uint32_t)ReadAddr, 0xFF);
...
SD_Error SD_ReadMultiBlocks(uint8_t* pBuffer, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  while (NumberOfBlocks--)
  {
    if(flag_SDHC == 1)
    {
      ...
      SD_SendCmd (SD_CMD_READ_SINGLE_BLOCK,(uint32_t)((ReadAddr + Offset) >> 9), 0xFF);
    }
    else
    {
      /*!< Send CMD17 (SD_CMD_READ_SINGLE_BLOCK) to read one block */
      SD_SendCmd (SD_CMD_READ_SINGLE_BLOCK, (uint32_t)(ReadAddr + Offset), 0xFF);
    }
...
SD_Error SD_WriteBlock(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize)
{
  ...
  SD_SendCmd(SD_CMD_WRITE_SINGLE_BLOCK, (uint32_t)(WriteAddr), 0xFF);
...
SD_Error SD_WriteMultiBlocks(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  while (NumberOfBlocks--)
  {
    if(flag_SDHC == 1)
    {
      ...
      SD_SendCmd(SD_CMD_WRITE_SINGLE_BLOCK, (uint32_t)((WriteAddr + Offset) >> 9), 0xFF);
    }
    else
    {
      /*!< Send CMD24 (SD_CMD_WRITE_SINGLE_BLOCK) to write blocks */
      SD_SendCmd(SD_CMD_WRITE_SINGLE_BLOCK, (uint32_t)(WriteAddr + Offset), 0xFF);
    }
...
SD_Error SD_GetCSDRegister(SD_CSD* SD_csd)
{
  ...
  #ifdef SHOW_DEBUG_PRINTF
  printf("Entering SD_GetCSDRegister() :\n");
  #endif
  ...
  if(flag_SDHC ==1)
  {
    ...
    /* Byte 8 */
    SD_csd->DeviceSize = ((uint16_t)CSD_Tab[8]) << 8;
    /* Byte 9 */
    SD_csd->DeviceSize |= CSD_Tab[9];
    
    SD_csd->DeviceSizeMul = 1;
    
  }
  
  #ifdef SHOW_DEBUG_PRINTF
  printf("  Device size : %16lX\n", SD_csd->DeviceSize);
  #endif
...
/**
  ...
  *         - SD_RESPONSE_NO_ERROR: Sequence succeed
  */
SD_Error SD_GetResponse(uint8_t AwaitedResponse)
{
  
  uint32_t Count = 0x0200; //Original: 0x00FF //Delay adjusted for different SD card timings
  uint8_t Response;
  ...
  do
  {
    Response = SD_ReadByte();
    Count--;
  } while ((Response != AwaitedResponse) && Count );
  
  if (Count == 0)
  {
    /*!< After time out */
    #ifdef SHOW_DEBUG_PRINTF
    printf("  SD respond timeout\n");
    #endif
...
{
  uint32_t RetryDownCounter = 6;
  SD_Error ResponseResult;

  #ifdef SHOW_DEBUG_PRINTF
  printf("Entering SD_GoIdleState() :\n");
  #endif
  
  uint8_t r1;
  uint16_t n2,n;
  uint32_t ocr;
  SD_Error retvalue = SD_RESPONSE_FAILURE;
  
  /*!< SD chip select low */
  SD_CS_LOW();
  
  // Try to put SD in SPI mode
  do
  {
    ...
    ResponseResult = SD_GetResponse(SD_IN_IDLE_STATE);
    
    RetryDownCounter--;
    
  } while((ResponseResult != SD_RESPONSE_NO_ERROR) && (RetryDownCounter != 0));
  
  if(ResponseResult != SD_RESPONSE_NO_ERROR)
  {
...
//  }
  ...
  /*----------checked the card version-----------*/
  #ifdef SHOW_DEBUG_PRINTF
  printf("  Check card revision :\n");
  #endif
  ...
  if(r1 == 0x05)
  {
    // We get here if we have a card that only supports SDv1 protocol
    #ifdef SHOW_DEBUG_PRINTF
    printf("  Version 1.X SD Memory card\n");
    #endif
    ...
    do
    {
      ...
      /*!< Send CMD41 (Activates the card process) until response equal to 0x0 */
  ...
  else if(r1 == 0x01)
  {
    ...
    do
    {
      ...
      SD_SendCmd(41, 0x40000000, 0); //Host supports SDHC
    ...
    else
    {
      ...
      if ((ocr & 0x40000000)==0){
        flag_SDHC=0; // SD
        #ifdef SHOW_DEBUG_PRINTF
        printf("flag_SDHC set to 0\n");
        #endif
      }else{
        #ifdef SHOW_DEBUG_PRINTF
        printf("flag_SDHC set to 1\n");
        #endif
        flag_SDHC=1; // SDHC
      }
...
/*
SD_Error SD_EnterPowerDown(void){
  //!< SD chip select low
  SD_CS_LOW();

  //!< Send CMD48 (CID register)
  SD_SendCmd(SD_CMD_SEND_CID, 0, 0xFF);
